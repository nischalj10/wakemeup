...
async def schedule_call(request: ScheduleCallRequest):
    ...
    # Calculate delay until the call time in seconds
    delay = (call_time - now).total_seconds()

    # Schedule the call
    scheduler.enter(delay, 1, make_call)

    return JSONResponse(status_code=200, content={"message": "Call scheduled successfully."})